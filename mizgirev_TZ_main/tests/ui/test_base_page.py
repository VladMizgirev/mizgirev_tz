...
from src.ui.locators.locators import CustomBasePageLocators, ProductListingPage, ChangeCityPageLocators, CartPageLocators
...
class TestCustomBasePage:
    ...
    def test_main_page_popular_category_card(self, base_page):
        ...
        with allure.step("Проверка на наличие всех карточек популярных категорий"):
            elements = base_page.get_elements_path(locator=CustomBasePageLocators.CATEGORY_CARD_ALL)[:4]
    ...
    def test_main_page_popular_category_card_click(self, base_page):
        ...
        with allure.step("Проверка на наличие всех карточек популярных категорий"):
            # берем только первые 4 категории, они стабильны
            elements = base_page.get_elements_path(locator=CustomBasePageLocators.CATEGORY_CARD_ALL)[:4]
            assert len(elements) == len(nav_items), "Нет всех карточек на странице"
            assert all(base_page.get_text_element(element=i) in nav_items for i in elements)
    ...
    @allure.description("Проверка наличия всех кнопок в шапке страницы")
    def test_header_nav_object_check(self, base_page):      # pass
        with allure.step("Проверка на наличие нужных элементов навигации на странице"):
            nav = header_nav_button_name
            header = base_page.get_elements_path(locator='//*[@id="__next"]/div/header/div[4]/div/div/div[2]/div[2]')
            elements = header[0].text.split('\n')
            assert len(elements) == 7, "Не отображается главная навигация по сайту"
            assert elements == nav, f"Ненайденные элементы: {set(nav).difference(elements)}"

    @allure.title("Главная страница - 004")
    @allure.description("Проверка меню смены города")
    def test_change_city_check(self, base_page):     # pass
    ...
    @allure.description("Проверка корзины")     # pass
